...
<form id="sdq-form" action="{{ route('storeSDQSecond') }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">SDQ 11 - 18 Tahun</span>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h4 class="text-title-sm 2xl:text-title-lg font-bold text-black dark:text-white">
                        Instrumen Strengths and Difficulties Questionnaire (SDQ)
                    </h4>
                    <span class="text-sm 2xl:text-base font-medium">Usia 11 - 18 Tahun</span>
                </div>
                <span>
                    <button type="submit"
                        class="w-[80px] h-[30px] md:w-[100px] md:h-[35px] text-sm rounded-[30px] bg-secondary text-primary duration-300 ease-linear dark:bg-white dark:text-secondary hover:text-white hover:bg-active dark:hover:bg-meta-4 dark:hover:text-white">Simpan</button>
                </span>
            </div>
        </div>
    </div>
    <div id="sdq-container" class="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-1">
        @if ($data->count() > 0)
            @include('admin.dashboard.partials.form.container.store-sdq-second')
        @endif
    </div>
    <div class="mt-4">
